#!/usr/bin/env python3
"""
Comprehensive Endpoint Security Test - FIXED VERSION
Tests all endpoints with both admin and regular users
"""
...
class EndpointSecurityTester:
    ...
    def test_authentication_scenarios(self):
        ...
        # Test 2: User login (will be created during test)
        self.print_result("User Login", True, "Testing user:user123 (will be created if needed)")
    ...
    def test_admin_endpoints(self):
        ...
        admin_endpoints = [
            ...
            {"method": "POST", "url": "/admin/users", "name": "Create User", "data": {"username": "testuser", "email": "test@test.com", "password": "test123", "role": "user"}},
        ...
        for endpoint in admin_endpoints:
            ...
            # Test with user auth (should fail for admin endpoints)
            try:
                if endpoint.get("method") == "GET":
    ...
    def test_user_endpoints(self):
        ...
        user_created = self.ensure_user_exists()
        ...
        for endpoint in user_endpoints:
            ...
            # Test with admin auth (should also work for user endpoints)
            try:
                if endpoint.get("method") == "GET":
                    response = self.admin_session.get(f"{self.base_url}{endpoint['url']}")
                elif endpoint.get("method") == "POST":
    ...
    def ensure_user_exists(self):
        """Ensure user:user123 exists, create if needed"""
        self.print_result("Ensure User Exists", True, "Checking if user:user123 exists")
        
        try:
            # Try to login as user first
            login_check = self.user_session.post(
                f"{self.base_url}/auth/login",
                json={"username": "user", "password": "user123"},
                headers={"Content-Type": "application/json"}
            )
            ...
            if login_check.status_code == 401:
                # User doesn't exist, create it with admin session
                self.print_result("Create User", True, "Creating user:user123 with admin session")
                create_response = self.admin_session.post(
                    f"{self.base_url}/admin/users",
                    json={"username": "user", "email": "user@localhost", "password": "user123", "role": "user"},
                    headers={"Content-Type": "application/json"}
                )
                
                if create_response.status_code == 201:
                    self.print_result("User Created", True, f"Status: {create_response.status_code}")
                    return True
                else:
                    self.print_result("User Creation", False, f"Status: {create_response.status_code}")
                    return False
            else:
                self.print_result("User Exists", True, "user:user123 already exists")
                return True
                
        except Exception as e:
            self.print_result("Ensure User Exists", False, f"Error: {str(e)}")
            return False
    ...
    def test_user_creation(self):
        """Test user creation as both admin and regular user"""
        self.print_section("USER CREATION")
        
        creation_results = {}
        
        # Test admin creates user
        self.print_result("Admin Creates User", True, "Testing admin creates testuser1")
        try:
            create_response = self.admin_session.post(
                f"{self.base_url}/admin/users",
            ...
            admin_creates_user = create_response.status_code == 201
            creation_results['admin_creates_user'] = admin_creates_user
            self.print_result("Admin Creates User", admin_creates_user, f"Status: {create_response.status_code}")
        except Exception as e:
            creation_results['admin_creates_user'] = False
            self.print_result("Admin Creates User", False, f"Error: {str(e)}")
        
        # Test user tries to create user (should fail)
        self.print_result("User Creates User", True, "Testing user tries to create testuser2")
        try:
            create_response = self.user_session.post(
                f"{self.base_url}/admin/users",
                json={"username": "testuser2", "email": "test2@test.com", "password": "test123", "role": "user"},
                headers={"Content-Type": "application/json"}
            )
            
            user_blocked = create_response.status_code == 403
            creation_results['user_blocked'] = user_blocked
            self.print_result("User Creates User", user_blocked, f"Status: {create_response.status_code}")
        except Exception as e:
    ...
    def run_comprehensive_test(self):
        ...
        print("This will test ALL endpoints with BOTH admin and regular users")
